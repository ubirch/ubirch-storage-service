...
class KeyValueStorage[T] {
  ...
  /**
    ...
    * @return
    */
  def fetchAll(limit: Int = 10000, sortedBy: Option[String] = None, order: String = "asc", filter: Option[String] = None) = storage.fetchAll(limit = limit, ordered = ordered, order = order, filter = filter)
